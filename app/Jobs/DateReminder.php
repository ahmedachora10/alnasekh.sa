...
class DateReminder implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach (Corp::with('user')->get() as $item) {
            ...
            $notification = $this->sendNotification($item->id, $status, $item->email, $item->thumbnail, [
                'title' => 'المنشأة ' . $status->name(),
                'content' => $item->name,
        ...
        foreach (EmployeeMedicalInsurance::with('employee.branch.corp.user')->get() as $item) {
            ...
            $notification = $this->sendNotification($item->id, $status, $item->employee->branch->corp->email, $item->employee->branch->corp->thumbnail, [
    ...
    private function sendNotification($id, $status, $email, $image = '', array $data = []) {
